<div id="confirmarModal" tabindex="-1"
    ...
    <div class="relative p-4 w-full max-w-md">
        ...
        <div
            ...
            <p class="text-2xl font-semibold text-center text-gray-800 mb-6">Seleccionar un Dueño</p>
            @if (count($duenosEcontrados) == 0)
                <p class="text-gray-500 text-center py-4">Sin coincidencias</p>
            @else
                @foreach ($duenosEcontrados as $dueno)
                    <button wire:click="selectDueno({{ $dueno->id }})"
                        ...
                        <div class="flex flex-col">
                            <div class="flex gap-1">
                                <div>
                                    <p class="text-sm font-medium text-gray-900">{{ $dueno->nombre }}</p>
                                    <p class="text-xs  text-gray-900">{{ $dueno->email }}</p>
                                </div>      
                                <div class="ml-4 ">
                                    <p class="text-sm font-medium text-gray-900">Mascotas</p>
                                    @php
                                        $mascotasDueno = $mascotas->where('dueno_id', $dueno->id);
                                    @endphp
                                    @foreach ($mascotasDueno as $mascotaDueno)
                                    <div class="flex">                                        
                                        <li class="text-xs">{{ $mascotaDueno->nombre }}</li>
                                        <img class="w-5 h-5 rounded-full ml-0.5" src="{{ asset("uploads/mascotas/$mascotaDueno->foto") }}" alt="">
                                    </div>                                         
                                    @endforeach
                                </div>                          
                            </div>
