            ...
            <tbody class="text-gray-800">
                @foreach ($ventas as $venta)
                    <tr wire:key='{{ $venta->id }}'
                        ...
                        <td class="px-4">
                            <div class="flex">
                                <p class="font-semibold mr-1">Nombre: </p><p>{{ $venta->cliente->nombre_rs }}</p>
                            </div>                          
                            <div class="flex">
                                <p class="font-semibold mr-1">RUC: </p><p>{{ $venta->cliente->ruc_ci }}</p>
                            </div>                                                               
                        </td>
                        <td class=" px-4">
                            @foreach ($movimientoP as $producto)
                                @if ($producto->venta_id == $venta->id)
                                    <div class="flex">
                                        <p class="font-semibold mr-1">{{ $producto->cantidad }}</p><p>{{ $producto->producto->nombre ?? '' }}</p>
